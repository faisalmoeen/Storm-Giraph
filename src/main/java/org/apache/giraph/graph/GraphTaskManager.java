...
public class GraphTaskManager<I extends WritableComparable, V extends Writable,
    ...
    public GraphTaskManager(Mapper<?, ?, ?, ?>.Context context) {
        ...
        this.isMaster = false;
        this.partitionID = context.getConfiguration().getInt("mapred.task.partition",-1);
    }
     ...
     */
    public void setup(Path[] zkPathList,int workerType)
        ...
        try {
            instantiateBspService(workerType);
        } catch (IOException e) {
    ...
    public void execute() throws IOException, InterruptedException {
        if(workerType==JAALConstants.SOURCE){
            if (checkTaskState()) {
                return;
            }
     ...
     */
    private void instantiateBspService(int workerType)
            throws IOException, InterruptedException {
        if (graphFunctions.isMaster()) {
            ...
            masterThread = new MasterThread<I, V, E>(serviceMaster, context,workerType);
            masterThread.start();
        }
        if (graphFunctions.isWorker()) {
            ...
            if(workerType== JAALConstants.WORKER) {
                serviceWorker = new BspServiceWorker<I, V, E>(context, this);
            }else if(workerType==JAALConstants.SOURCE){
                serviceWorker = new BspServiceSource<>(context, this);
            }
            if (LOG.isInfoEnabled()) {
